...
Supports multi-GPU training with DeepSpeed ZeRO via Accelerate.
Uses full precision (no quantization) - DeepSpeed handles memory through parameter sharding.
...
def train_model(cfg, model, tokenizer, train_data, val_data, save_dir: str, use_lora: bool):
    """Tokenize datasets, configure Trainer, and run fine-tuning with DeepSpeed."""
    ...
    # DeepSpeed-specific training arguments
    args = TrainingArguments(
        ...
        # DeepSpeed config is handled by Accelerate
        deepspeed=None,  # Accelerate will inject the config
    )
    ...
    print(f"\n🎯 Starting fine-tuning with {num_gpus} GPU(s) using DeepSpeed...")
...
def main():
    ...
    # Initialize Accelerator for DeepSpeed
    ...
    # Determine DeepSpeed ZeRO stage for folder naming
    deepspeed_plugin = accelerator.state.deepspeed_plugin
    if deepspeed_plugin and hasattr(deepspeed_plugin, "deepspeed_config"):
        ds_config = deepspeed_plugin.deepspeed_config
        zero_stage = ds_config.get("zero_optimization", {}).get("stage", 0)
        config_folder = f"deepspeed_zero{zero_stage}_{num_gpus}gpu_{training_type}"
    else:
        # Fallback if we can't determine stage
        config_folder = f"deepspeed_{num_gpus}gpu_{training_type}"

    run_output_dir = os.path.join(OUTPUTS_DIR, config_folder, model_name)
    run_name = f"{config_folder}-{model_name}"

    print(f"\n🔧 Training mode: DeepSpeed with {num_gpus} GPU(s)")
    print(f"📁 Output directory: {run_output_dir}")

    # Load dataset
    train_data, val_data, _ = load_and_prepare_dataset(cfg)

    # Setup model WITHOUT quantization (DeepSpeed handles memory through sharding)
    model, tokenizer = setup_model_and_tokenizer(
        cfg,
        use_4bit=False,  # No quantization for DeepSpeed
